...
def read_audio(file, multisrc, random_channel, pad_to):
    ...
    mixture, fs = torchaudio.load(file) # type: ignore
...
class StronglyAnnotatedSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.embeddings_hdf5_file is not None:
            ...
            global_embeddings = torch.from_numpy(self.hdf5_file["global_embeddings"][index]).float() # type: ignore
            frame_embeddings = torch.from_numpy(np.stack(self.hdf5_file["frame_embeddings"][index])).float() # type: ignore
